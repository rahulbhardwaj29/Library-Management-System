...
class StudentClass:
    ...
    def __init__(self,hwindow):
        ...
        self.window.title("Library Management\Student")
        ...
        self.L9 = Label(self.window,text='Book Issued',background=mycolor1,font=myfont1)
        ...
        self.v2 = StringVar()
        self.c1 = Combobox(self.window,values=['IT','BBA'],textvariable=self.v2,font=myfont1,state='readonly')
        # self.c1.bind("<<ComboboxSelected>>",lambda e: self.getAllCourses())

        self.v3 = StringVar()
        self.c2 = Combobox(self.window,values=['BTech','BCA'],textvariable=self.v3,font=myfont1,state='readonly')
        self.t9 = Entry(self.window, font=myfont1)


        # ------------------ table ---------------------
        self.mytable1 = Treeview(self.window, columns=['c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7', 'c8', "c9"], height=10)
        ...
        self.mytable1.heading('c9', text='Book Issued')
        self.mytable1['show'] = 'headings'

        self.mytable1.column('c1', width=90, anchor='center')
        self.mytable1.column('c2', width=90, anchor='center')
        self.mytable1.column('c3', width=90, anchor='center')
        self.mytable1.column('c4', width=90, anchor='center')
        self.mytable1.column('c5', width=90, anchor='center')
        self.mytable1.column('c6', width=90, anchor='center')
        self.mytable1.column('c7', width=90, anchor='center')
        self.mytable1.column('c8', width=90, anchor='center')
        ...
        self.t9.place(x=x1 + x_diff, y=y1)
        y1+=y_diff
    ...
    def updateData(self):
        ...
        try:
            ...
            qry = 'update student set name=%s, phone=%s,gender=%s, dob=%s,' \
                  ' address=%s, department=%s,course=%s,book_issued=%s,pic = %s where rollno=%s'
            rowcount = self.curr.execute(qry ,( self.t2.get(),self.t3.get(),
                    self.v1.get(),self.t5.get_date(),self.t6.get('0.0',END).strip(),
                    self.v2.get(),self.v3.get(),self.t9.get(),self.actualname, self.t1.get()) )
            self.conn.commit()
            if rowcount==1:
                #--------- update image ---------------
    ...
    def fetchData(self,pcolumn=None):
        ...
        try:
            ...
            if rowdata:
                ...
                self.t9.insert(0, rowdata[8])
                self.actualname = rowdata[9]
                self.oldname = rowdata[9]
    ...
    def validationCheck(self):
        if not self.t1.get().isdigit():
            ...
            return False
        elif len(self.t9.get())<2:
            messagebox.showwarning("Validation","Please Enter Bookname\n(atleast 2 characters)",parent=self.window)
            return False
        return True
